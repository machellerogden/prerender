...
util.getUrl = function(req) {
    ...
    try {
        ...
        decodedUrl = url.parse(req.url);
        decodeUrl = decodedUrl.path.slice(1);
        console.log("Search: ", decodedUrl.search)
    ...
    if (parts.query && parts.query.hasOwnProperty('_escaped_fragment_')) {
        if (parts.query['_escaped_fragment_']) parts.hash = '#!' + parts.query['_escaped_fragment_'] + parts.query.slice(1, parts.query.length);
